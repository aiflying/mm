<?xml version="1.0" encoding="UTF-8"?>
<map version="0.9.0">
<!-- This file is saved using a hacked version of FreeMind. visit: http://freemind-mmx.sourceforge.net -->
<!-- Orignal FreeMind, can download from http://freemind.sourceforge.net -->
<!-- This .mm file is CVS/SVN friendly, some atts are saved in .mmx file. (from ossxp.com) -->
<node ID="ID_87719608" 
	TEXT="2010北京通讯展">
<node FOLDED="true" ID="ID_1168990721" POSITION="left" 
	TEXT="key point">
<icon BUILTIN="flag"/>
<node ID="ID_781503284" 
	TEXT="三屏融合、内容共享">
<icon BUILTIN="messagebox_warning"/>
</node>
<node ID="ID_1170928930" 
	TEXT="云"/>
<node ID="ID_1118991806" 
	TEXT="物联网"/>
</node>
<node FOLDED="true" ID="ID_1676463417" POSITION="left" 
	TEXT="运营商">
<node FOLDED="true" ID="ID_1975180036" 
	TEXT="中国移动">
<node ID="ID_1830970744" 
	TEXT="宜居通"/>
<node ID="ID_1347772030" 
	TEXT="物联网"/>
</node>
<node FOLDED="true" ID="ID_1084651755" 
	TEXT="中国电信">
<node ID="ID_6906333" 
	TEXT="翼机通（彩门）"/>
<node ID="ID_82832125" 
	TEXT="信息机"/>
<node ID="ID_1016100200" 
	TEXT="ishare"/>
<node FOLDED="true" ID="ID_1597079430" 
	TEXT="5A家庭">
<node ID="ID_1546615679" 
	TEXT="IPTV，与楼盘合作（如江门某小区）"/>
<node ID="ID_63957103" 
	TEXT="可视电话"/>
</node>
</node>
<node FOLDED="true" ID="ID_515480933" 
	TEXT="中国联通">
<node FOLDED="true" ID="ID_448966531" 
	TEXT="VDC，成都云点科技给做的">
<node ID="ID_1191367531" 
	TEXT="瘦终端概念"/>
</node>
</node>
</node>
<node FOLDED="true" ID="ID_213674782" POSITION="left" 
	TEXT="芯片厂商">
<node FOLDED="true" ID="ID_533472557" 
	TEXT="MTK">
<node ID="ID_1134248342" 
	TEXT="拍照自动拼接软件"/>
</node>
<node FOLDED="true" ID="ID_1362364468" 
	TEXT="Rickchip">
<node ID="ID_556478658" 
	TEXT="各种手机、各种pad"/>
<node ID="ID_106195015" 
	TEXT="3D芯片2816，效果还不太好"/>
</node>
<node FOLDED="true" ID="ID_676007439" 
	TEXT="Qualcomn">
<node FOLDED="true" ID="ID_1741378674" 
	TEXT="Snapdragon系列芯片">
<node FOLDED="true" ID="ID_893330475" 
	TEXT="QST8650">
<node ID="ID_1018485146" 
	TEXT="华为S7、DELL MINI使用"/>
<node ID="ID_825793920" 
	TEXT="带编码"/>
</node>
</node>
</node>
</node>
<node FOLDED="true" ID="ID_1917119526" POSITION="right" 
	TEXT="方案及产品厂商">
<node FOLDED="true" ID="ID_310890981" 
	TEXT="华为">
<node FOLDED="true" ID="ID_1238782828" 
	TEXT="SingleCloud">
<node ID="ID_108512835" 
	TEXT="针对小型企业和分布式办公"/>
</node>
<node ID="ID_540556084" 
	TEXT="云盘"/>
</node>
<node FOLDED="true" ID="ID_1906186931" 
	TEXT="中兴">
<node ID="ID_1566865631" 
	TEXT="物联网"/>
</node>
<node FOLDED="true" ID="ID_409687490" 
	TEXT="同洲">
<richcontent TYPE="NOTE"><html>
  <head>
    
  </head>
  <body>
    <p>
      同洲电子已从2006年开始从“机顶盒生产厂商”向“广电综合业务解决方案提供商”转型。2008年，同洲电子开始提供端到端的一系列的解决方案，比如从前端系统到承载网，从接入到终端。
    </p>
  </body>
</html>
</richcontent>
<node FOLDED="true" ID="ID_598525580" STYLE="bubble" 
	TEXT="“我甩我帅”">
<node ID="ID_637424636" 
	TEXT="概念：移动和固定屏幕之前的视频衔接"/>
<node ID="ID_1345804910" 
	TEXT="应用场景：实际上业务场景很简单，我们坐在家里的沙发上看电视，这时突然有人按门铃，需要起身开门，但正好又演到了关键镜头，比如关键射门或投篮，不想错过，那怎么办呢？我拿起Pad甩一下，这时整个视频就从电视上到了我的手机上或Pad上，这时我可以拿着它走到门口，在离开电视机前的情况下仍然不错过精彩片断，打开门让家人、客人进了屋，再回到沙发前，但我用手机的体验肯定没办法和大屏幕的体验相比，这时我再一甩，手机的视频继续回到电视上播放。我们给它起了一个好听的名字，“我甩我帅”。"/>
</node>
<node FOLDED="true" ID="ID_1022267441" 
	TEXT="手机遥控">
<node ID="ID_1289359344" 
	TEXT="在同洲手机上开发的软件，一个全屏的遥控器"/>
</node>
<node ID="ID_385690832" 
	TEXT="物联网"/>
<node FOLDED="true" ID="ID_1103245694" STYLE="bubble" 
	TEXT="视频共享">
<node ID="ID_1425948137" 
	TEXT="概念：媒体文件共享"/>
<node ID="ID_305241074" 
	TEXT="应用场景：机顶盒、手机等设备都有“用户名”，媒体文件可以选择“好友列表”中的设备进行发送。本地播放与发送文件是两个不干扰的进程。"/>
</node>
<node FOLDED="true" ID="ID_155061684" 
	TEXT="其它三网融合的应用">
<node ID="ID_599466880" 
	TEXT="视频通讯"/>
<node ID="ID_387996226" 
	TEXT="短信、彩信"/>
</node>
<node FOLDED="true" ID="ID_1935850140" 
	TEXT="三网融合">
<node ID="ID_1313863546" 
	TEXT="同洲负责的试点城市：哈尔滨、武汉、深圳"/>
</node>
</node>
<node FOLDED="true" ID="ID_880092041" 
	TEXT="三星">
<node ID="ID_1266243626" 
	TEXT="娱乐共享"/>
<node ID="ID_637291838" 
	TEXT="Moblile TV/Game"/>
</node>
<node FOLDED="true" ID="ID_215495227" 
	TEXT="UTStarcom">
<node FOLDED="true" ID="ID_177820501" 
	TEXT="“奔流”IPTV全套方案">
<node ID="ID_537384894" 
	TEXT="三网融合试点--云浮">
<icon BUILTIN="help"/>
</node>
</node>
<node FOLDED="true" ID="ID_1602947896" 
	TEXT="乐炫 &quot;Le Shine&quot; 全功能家庭数字多媒体中心">
<node ID="ID_620948193" 
	TEXT="支持泰山游戏"/>
<node ID="ID_995485057" 
	TEXT="视频通讯、电视短信，个人、公共视频监控等各种视频信息的接入"/>
<node ID="ID_1307994051" 
	TEXT="3D 图片墙"/>
<node ID="ID_19870048" 
	TEXT="非全格式解码"/>
<node ID="ID_175392976" 
	TEXT="Wifi-Smart Touch 遥控"/>
<node ID="ID_1636854743" 
	TEXT="体感游戏手柄遥控"/>
<node ID="ID_1295446829" 
	TEXT="基于 mini-第4屏技术的可视遥控器"/>
<node ID="ID_1787165925" 
	TEXT="Atom 3300 芯片"/>
<node ID="ID_1102213946" 
	TEXT="Linux / Windows XPE"/>
<node ID="ID_795071195" 
	TEXT="Nvidia GeForce 9400集成芯片组"/>
</node>
</node>
<node FOLDED="true" ID="ID_1026063313" 
	TEXT="飞扬无限">
<node FOLDED="true" ID="ID_1855445574" 
	TEXT="智能家居">
<node ID="ID_1922229259" 
	TEXT="射频433技术，控制家庭电气设备"/>
</node>
<node ID="ID_765659521" 
	TEXT="3G 终端"/>
</node>
<node FOLDED="true" ID="ID_1353072046" 
	TEXT="NSN">
<node ID="ID_519870397" 
	TEXT="多屏互动"/>
<node ID="ID_35749419" 
	TEXT="对不同终端做不同的编码，再从服务器下发"/>
</node>
</node>
<node FOLDED="true" ID="ID_1241369221" POSITION="left" 
	TEXT="几个应用软件开发公司">
<node FOLDED="true" ID="ID_1759004604" 
	TEXT="播思">
<icon BUILTIN="flag-blue"/>
<node ID="ID_1976479110" 
	TEXT="端到端android软件解决方案，移动Ophone是播思做的"/>
</node>
<node ID="ID_1867651829" 
	TEXT="数字天域--好管家"/>
<node ID="ID_1210231476" 
	TEXT="活力天汇--冒泡网"/>
<node ID="ID_673639638" 
	TEXT="金盛四海--信息机全套解决方案"/>
<node ID="ID_530421779" 
	TEXT="闪动科技--android手机软件开发"/>
<node ID="ID_861663858" 
	TEXT="视频中国--手机电视"/>
</node>
</node>
</map>
